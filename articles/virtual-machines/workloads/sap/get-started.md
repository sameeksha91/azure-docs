...
Microsoft Azure offerings for SAP workload:
1) SAP HANA on Azure (Large Instances)
2) SAP HANA on Azure Virtual Machines
3) SAP NetWeaver on Azure Virtual Machines

Besides hosting SAP NetWeaver scenarios with the different DBMS on Azure, you can host different other SAP workload scenarios, like SAP BI on Azure. Documentation regarding SAP NetWeaver deployments on Azure native Virtual Machines can be found below in the section "3) SAP NetWeaver on Azure Virtual Machines."

Azure has native Azure Virtual Machine offers that are ever growing in size of CPU and memory resources to cover SAP workload that leverages SAP HANA. For more information on this topic, look below under the section "2) SAP HANA on Azure Virtual Machines."

The uniqueness of Azure for SAP HANA is a unique offer that sets Azure apart from competition. In order to enable hosting more memory and CPU resource demanding SAP scenarios involving SAP HANA, Azure offers the usage of customer dedicated bare-metal hardware for the purpose of running SAP HANA deployments that require up to 20 TB (60 TB scale-out) of memory for S/4HANA or other SAP HANA workload. This unique Azure solution of SAP HANA on Azure (Large Instances) allows you to run SAP HANA on the dedicated bare-metal hardware with the SAP application layer or workload middle-ware layer hosted in native Azure Virtual Machines. This solution is documented as several different documents below under the heading "1) SAP HANA on Azure (Large Instances)."   
...
##### 1.1) Overview and architecture of SAP HANA on Azure (Large Instances)
...
##### 1.2) Infrastructure and connectivity to SAP HANA on Azure (Large Instances)
...
##### 1.3) Install SAP HANA in SAP HANA on Azure (Large Instances)
...
##### 1.4) High availability and disaster recovery of SAP HANA on Azure (Large Instances)
...
##### 1.5) Troubleshooting and monitoring of SAP HANA on Azure (Large Instances)
...
##### 2.1) Getting started with SAP HANA on Azure
...
##### 2.2) S/4HANA SAP CAL deployment on Azure
...
##### 2.3) High Availability of SAP HANA in Azure Virtual Machines
...
##### 2.4) SAP HANA backup overview on Azure VMs
...
##### 2.5) SAP HANA file level backup on Azure VMs
...
##### 2.6) SAP HANA snapshot based backups on Azure VMs
...
## 3) SAP NetWeaver deployed on Azure Virtual Machines

##### 3.1) Deploy SAP IDES system on Windows and SQL Server through SAP CAL on Azure
...
##### 3.2) Quickstart guide for NetWeaver on SUSE Linux on Azure
Title: Testing SAP NetWeaver on Microsoft Azure SUSE Linux VMs
...
##### 3.3) <a name="3da0389e-708b-4e82-b2a2-e92f132df89c"></a>Planning and implementation
...
##### 3.4) High Availability configurations of SAP NetWeaver in Azure VMs
...
##### 3.5) Realizing Multi-SID deployments of SAP NetWeaver in Azure VMs
...
##### 3.6) <a name="6aadadd2-76b5-46d8-8713-e8d63630e955"></a>Deployment of SAP NetWeaver in Azure VMs
...
##### 3.7) <a name="1343ffe1-8021-4ce6-a08d-3a1553a4db82"></a>DBMS deployment guide
...
##### 3.8) Using Azure Site Recovery for SAP workload
